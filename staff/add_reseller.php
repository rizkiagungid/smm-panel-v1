		...
		if (check_empty($validation) == true) {
			...
			if ($model->db_query($db, "username", "users", "username = '".mysqli_real_escape_string($db, $input_post['username'])."'")['count'] > 0) {
				...
				if ($model->db_insert($db, "users", $input_post) == true) {
					...
					$model->db_insert($db, "balance_logs", array('user_id' => $login['id'], 'type' => 'minus', 'amount' => $config['register']['price']['reseller'], 'note' => 'Mendaftarkan Member. Username: '.$input_post['username'].'.', 'created_at' => date('Y-m-d H:i:s')));
					$_SESSION['result'] = array('alert' => 'success', 'title' => 'Reseller berhasil didaftarkan!', 'msg' => '<br />Username: '.$input_post['username'].'<br />Password: '.$validation['password']);
				} else {
...
?>
						...
						<div class="row">
							<div class="offset-lg-3 col-lg-6">
								<div class="card-box">
									<h4 class="m-t-0 m-b-30 header-title"><i class="fa fa-plus"></i> Tambah Reseller</h4>
<div class="alert alert-info"><b>Biaya Pendaftaran:</b> Rp <?php echo number_format($config['register']['price']['reseller'],0,',','.') ?><br /><b>Bonus Saldo:</b> Rp <?php echo number_format($config['register']['bonus']['reseller'],0,',','.') ?></div>
